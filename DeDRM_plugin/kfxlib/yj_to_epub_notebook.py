...
class KFX_EPUB_Notebook(object):
    ...
    def scribe_notebook_stroke(self, content, parent, location_id):
        ...
        if nmdl_type == "nmdl.stroke_group":
            ...
            for annotation in content.pop("$683", []):
                self.scribe_notebook_annotation(annotation, group_elem)
            ...
            nmdl_origin_stroke_id = content.pop("nmdl.origin_stroke_id", None)
            if False:
                log.info("origin_stroke_id=%s" % nmdl_origin_stroke_id)
            ...
            if nmdl_color in STROKE_COLORS:
                stroke_color_name, stroke_color = STROKE_COLORS[nmdl_color]
            else:
            ...
            desc_elem.text = "%s %s %s" % (thickness_name, stroke_color_name, brush_name)
            ...
            if False:
                ...
                group_elem.set("fill", self.color_str(stroke_color))

            else:
                group_elem.set("fill", "none")
                group_elem.set("stroke", self.color_str(stroke_color))
            ...
            if False:
                ...
                with disable_debug_log():
                    png = Image.new("P" if stroke_color != 0 else "1", (png_width, png_height))

                    if png.mode == "P":
                        png.putpalette(
                            [(stroke_color >> 16) & 255, (stroke_color >> 8) & 255, stroke_color & 255, 0, 0, 0],
                            rawmode="RGB")
    ...
    def scribe_notebook_annotation(self, annotation, elem):
        annotation_type = annotation.pop("$687")
        if annotation_type == "nmdl.hwr":
            story = self.get_named_fragment(annotation, ftype="$259", name_symbol="$749")

            story_name = story.pop("$176")
            self.push_context("story %s" % story_name)
            self.get_location_id(story)

            for content in story.pop("$146", []):
                self.scribe_annotation_content(content, elem)

            self.pop_context()
            self.check_empty(story, self.content_context)
            self.check_empty(annotation, "%s annotation" % self.content_context)
        else:
            log.error("%s has unexpected annotation_type: %s" % (self.content_context, annotation_type))

    def scribe_annotation_content(self, content, elem):
        data_type = ion_type(content)

        if data_type is IonSymbol:
            self.process_content(self.get_fragment(ftype="$608", fid=content))
            return
        ...
        location_id = self.get_location_id(content)
        self.push_context(location_id)

        content_type = content.pop("$159", None)
        if content_type == "$269":
            word_iteration_type = content.pop("$605", None)
            if word_iteration_type is not None and word_iteration_type != "model":
                log.warning("%s has text word_iteration_type=%s" % (self.content_context, word_iteration_type))

            top = content.pop("$58", 0.0)
            left = content.pop("$59", 0.0)
            content.pop("$57")
            content.pop("$56")
            text = content.pop("$145", "")

            desc_elem = etree.SubElement(elem, "desc")
            desc_elem.text = text

            text_elem = etree.SubElement(elem, "text", attrib={
                "x": "%d" % left, "y": "%d" % top, "stroke": "none",
                "fill": "red", "opacity": "%0.2f" % ANNOTATION_TEXT_OPACITY})

            for style_event in content.pop("$142", []):
                model = style_event.pop("$604", None)
                if model is not None and model != "word":
                    log.warning("%s has text model=%s" % (self.content_context, model))

                offset = style_event.pop("$143")
                length = style_event.pop("$144")
                word_top = style_event.pop("$58", 0.0)
                word_left = style_event.pop("$59", 0.0)
                word_height = style_event.pop("$57")
                word_width = style_event.pop("$56")
                style_event.pop("$749", None)
                word = text[offset:offset + length]

                if word:
                    tspan_elem = etree.SubElement(text_elem, "tspan", attrib={
                        "x": "%d" % word_left, "y": "%d" % (word_top + (word_height / 2)),
                        "textLength": "%d" % word_width, "font-size": "%d" % ((word_width * 2) / len(word)),
                        "dominant-baseline": "middle"})
                    tspan_elem.text = word

                self.check_empty(style_event, "%s style_event" % self.content_context)
        else:
            log.error("%s unknown annotation content type: %s" % (self.content_context, content_type))

        self.pop_context()
        self.check_empty(content, "%s content" % self.content_context)
